...
同様で，この時点で残った故障候補が以下のようになった．
\begin{itemize}
   \item コマンドリンク35：カメラヒータ‐カメラ間
   \item テレメトリリンク43：カメラ‐カメラ温度計間
   \item テレメトリリンク27：カメラ温度計‐TOBC間
\end{itemize}
\begin{figure}[H]
   \centering
...
\end{figure}
この時，システムによって洗い出された検証用のコマンドは以下の図\ref{fig:mode2_COM_candidate}
のようになり，それぞれの指標の値を見ると


\begin{figure}[H]
   \centering
      \includegraphics[height=2cm]{figure/COM15_27_TEL18_COM_candidate.png}
      \caption{検証用コマンド探索結果}
      \label{fig:mode2_COM_candidate}
\end{figure}
